...
def finish_preprocessing(image, mask, output_dir, track_processing):
    ...
    voxel_course_dict, _ = generate_time_courses(image_array, mask_array)
    funtimes = install_r_packages()

    detrended_image_path = multi_detrending(voxel_course_dict, funtimes, output_dir)
...
def multi_detrending(voxel_course_dict, funtimes, output_dir):
    # Prepare numpy array to put values into
    detrended_map = np.empty((91, 109, 91, 478))

    detrended_map[:] = 0.

    shm = shared_memory.SharedMemory(create=True, size=detrended_map.nbytes)

    shm_detr_map = np.ndarray(detrended_map.shape, dtype=detrended_map.dtype, buffer=shm.buf)

    shm_detr_map[:] = detrended_map[:]

    dtype = shm_detr_map.dtype
    shape = shm_detr_map.shape
    name = shm.name

    # First step split voxel vourse dict into 8 parts
    dict_length = len(voxel_course_dict)

    print("DICT_LENGTH:", dict_length)

    twentieth_length = dict_length / 20

    length_list = [int(twentieth_length * i) for i in range(1, 21)]

    print("LENGTH LIST:", length_list, "\n\n")

    i = 0

    voxels_to_detrend_list = []

    for length in length_list:

        voxels_to_detrend = list(voxel_course_dict.keys())[i:length]

        voxels_to_detrend_dict = {voxel: voxel_course_dict[voxel] for voxel in voxels_to_detrend}

        voxels_to_detrend_list.append(voxels_to_detrend_dict)
        
        i = length

    captured_voxels = 0

    for dic in voxels_to_detrend_list:
        captured_voxels += len(dic)

    print("VOXELS TO DETREND CAPTURED:", captured_voxels)


    # Define our multiple processes
    p1 = Process(target=spline_detrending, args=(voxels_to_detrend_list[0], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p2 = Process(target=spline_detrending, args=(voxels_to_detrend_list[1], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p3 = Process(target=spline_detrending, args=(voxels_to_detrend_list[2], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p4 = Process(target=spline_detrending, args=(voxels_to_detrend_list[3], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p5 = Process(target=spline_detrending, args=(voxels_to_detrend_list[4], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p6 = Process(target=spline_detrending, args=(voxels_to_detrend_list[5], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p7 = Process(target=spline_detrending, args=(voxels_to_detrend_list[6], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p8 = Process(target=spline_detrending, args=(voxels_to_detrend_list[7], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p9 = Process(target=spline_detrending, args=(voxels_to_detrend_list[8], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p10 = Process(target=spline_detrending, args=(voxels_to_detrend_list[9], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p11 = Process(target=spline_detrending, args=(voxels_to_detrend_list[10], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p12 = Process(target=spline_detrending, args=(voxels_to_detrend_list[11], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p13 = Process(target=spline_detrending, args=(voxels_to_detrend_list[12], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p14 = Process(target=spline_detrending, args=(voxels_to_detrend_list[13], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p15 = Process(target=spline_detrending, args=(voxels_to_detrend_list[14], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p16 = Process(target=spline_detrending, args=(voxels_to_detrend_list[15], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p17 = Process(target=spline_detrending, args=(voxels_to_detrend_list[16], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p18 = Process(target=spline_detrending, args=(voxels_to_detrend_list[17], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p19 = Process(target=spline_detrending, args=(voxels_to_detrend_list[18], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    p20 = Process(target=spline_detrending, args=(voxels_to_detrend_list[19], funtimes, detrended_map, name, dtype, shape, output_dir, ))
    ...
    # Make rest of the program wait until the multiple processes are finished
    p1.join()
    p2.join()
    p3.join()
    p4.join()
    ...
    detrended_image = nib.Nifti1Image(shm_detr_map, AFFINE_MATRIX)
    detrended_image_path = f"{output_dir}/detrended_image.nii.gz"
    nib.save(detrended_image, detrended_image_path)

    return detrended_image_path
...
def spline_detrending(voxel_course_dict, funtimes, detrended_map, name, dtype, shape, output_dir):

    existing_shm = shared_memory.SharedMemory(name=name)

    detrended_map = np.ndarray(shape, dtype=dtype, buffer=existing_shm.buf)
    ...
    for voxel in voxel_course_dict:
        ...
        trend_test_result = funtimes.notrend_test(time_course, test="WAVK", factor_length="user.defined", Window=20)

        p_value = trend_test_result[3].item()

        if p_value < 0.05:
            print("P_VALUE:", p_value, flush=True)
            print("COORDS:", x, y, z, flush=True)

    #quarter_floats = np.linspace(120, 360, 3)

    #quarter_ints = [int(x) for x in quarter_floats]

    #reshaped_quarters = np.split(time_course, quarter_ints)

    # Fill it out
    #reshaped_quarters[-1] = np.append(reshaped_quarters[-1], np.nanmean(reshaped_quarters[-1]))
    #reshaped_quarters[-1] = np.append(reshaped_quarters[-1], np.nanmean(reshaped_quarters[-1]))

    #sum_diffs = 1000

    #sum_diffs = (np.nanmean(reshaped_quarters[0]) - np.nanmean(reshaped_quarters[1])) +\
                #(np.nanmean(reshaped_quarters[1]) - np.nanmean(reshaped_quarters[2])) +\
                #(np.nanmean(reshaped_quarters[2]) - np.nanmean(reshaped_quarters[3]))
...
def detrending(voxel_course_dict, output_dir, track_processing):
    ...
    #detrended_image = image.clean_img(smoothed_image_path, detrend=True, standardize=False)
